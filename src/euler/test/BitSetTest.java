...
public class BitSetTest {
	...
	public void andOrTest() {
		for (int length = 0; length < BitSetTest.MAX_LENGTH; length++) {
			BitSet empty = new BitSet(length);
			BitSet even = generateEven(length), odd = generateOdd(length);
			for (int i = 0; i < length; i++) {
				((i & 1) == 0 ? even : odd).set(i);
			}
			BitSet full = even.or(odd);
			BitSet empty2 = even.and(odd);
	...
	public void cloneTest() {
		for (int length = 0; length < BitSetTest.MAX_LENGTH; length++) {
			BitSet[] bss = generateArray(length);
			for (BitSet bs : bss) {
	...
	private BitSet[] generateArray(int length) {
		return new BitSet[] {
		                     new BitSet(length), generateEven(length), generateFull(length), generateOdd(length)
		};
	}
	...
	public void intersectTest() {
		for (int length = 0; length < BitSetTest.MAX_LENGTH; length++) {
			BitSet even = generateEven(length), odd = generateOdd(length), full = generateFull(length), empty = new BitSet(
				length);
	...
	public void isSetTest() {
		for (int length = 0; length < BitSetTest.MAX_LENGTH; length++) {
			BitSet even = generateEven(length);
			for (int i = 0; i < length; i++) {
				Assert.assertEquals((i & 1) == 0, even.isSet(i));
			}

			BitSet odd = generateOdd(length);
			for (int i = 0; i < length; i++) {
