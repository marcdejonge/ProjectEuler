...
public class BitSet implements Cloneable {
	...
	public BitSet(int length) {
		int nrInts = (length >>> BitSet.SHIFT) + ((length & BitSet.MASK) == 0 ? 0 : 1);
		words = new long[nrInts];
		this.length = length;
	}

	public BitSet and(BitSet other) {
		BitSet result = new BitSet(Math.min(length, other.length));
	...
	public void flip(int from, int to) {
		...
		int fromWordIndex = from >>> BitSet.SHIFT, toWordIndex = to >>> BitSet.SHIFT;
	...
	public void setLength(int length) {
		length++;
		int newWordLength = (length >>> BitSet.SHIFT) + ((length & BitSet.MASK) == 0 ? 0 : 1);
