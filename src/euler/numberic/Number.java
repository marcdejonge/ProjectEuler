...
public class Number extends java.lang.Number implements Comparable<Number> {
	...
	public Number add(Number other) {
		Number res = new Number(values.length);
	...
	public Number addInc(Number other) {
		...
		if (carry > 0) {
			Number r = new Number(values.length + 1);
	...
	public Number concat(Number nr) {
		Number res = new Number(values.length + nr.values.length);
	...
	public Number multiply(Number other) {
		int length = getFilledLength() + other.getFilledLength();
	...
	public Number replace(byte digit, byte replacement) {
		Number number = new Number(values.length);
		for (int i = 0; i < values.length; i++) {
			if (values[i] == digit) {
	...
	public String toString() {
		...
		char[] chars = new char[getFilledLength()];
