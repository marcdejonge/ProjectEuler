...
public class HarshadNumbers extends AbstractSequence {
    ...
    private final Queue<JavaPair<Long, Integer>> harshadNumbers = new LinkedList<>();
    ...
    public HarshadNumbers(BiFunction<Long, Integer, Boolean> extraCheck) {
        ...
        for (int ix = 1; ix < 10; ix++) {
            harshadNumbers.add(JavaPair.from(Long.valueOf(ix), ix));
        }
    }
    ...
    public long next() {
        while (!harshadNumbers.isEmpty()) {
            JavaPair<Long, Integer> prev = harshadNumbers.poll();
            ...
            for (int ix = 0; ix < 10; ix++) {
                ...
                if (newNr > 0 && newNr / newSumDigits * newSumDigits == newNr) {
                    harshadNumbers.add(JavaPair.from(newNr, newSumDigits));
                }
            }
